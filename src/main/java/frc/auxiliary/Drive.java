...
import frc.HardwareInterfaces.Transmission.TankTransmission.Motors;
...
public class Drive
     ...
     */
    public Drive(TankTransmission transmission, KilroyEncoder leftEncoder,
     ...
     */
    public Drive(TankTransmission transmission, KilroyEncoder leftRearEncoder,
            ...
            double accelerationTimeInSecs)
    {
        ...
        if (getEncoderDistanceAverage(Motors.ALL) > arcLength)
            {
        ...
        double leftRate = getEncoderRate(Motors.LEFT);
        double rightRate = getEncoderRate(Motors.RIGHT);
    ...
    public boolean brake(BrakeType type)
    {
        ...
        if (this.currentBrakeIteration == 1)
            ...
            brakeDeltas[0] = getEncoderTicks(Motors.LEFT_REAR)
                    - brakePrevEncoderVals[0];
            brakeDeltas[1] = getEncoderTicks(Motors.RIGHT_REAR)
            ...
            if (this.encoders.length > 2)
                {
                brakeDeltas[2] = getEncoderTicks(Motors.LEFT_FRONT)
                        - brakePrevEncoderVals[2];
                brakeDeltas[3] = getEncoderTicks(Motors.RIGHT_FRONT)
        ...
        if ((this.brakeInitialDirection[0] != Math
                .signum(getEncoderTicks(Motors.LEFT_REAR)))
                || (this.brakeInitialDirection[1] != Math
                        .signum(getEncoderTicks(Motors.RIGHT_REAR))))
            finished = true;
        if ((this.brakeInitialDirection.length > 2)
                && ((this.brakeInitialDirection[2] != Math
                        .signum(getEncoderTicks(Motors.LEFT_FRONT))
                        || (this.brakeInitialDirection[3] != Math
                                .signum(getEncoderTicks(Motors.RIGHT_FRONT))))))
            finished = true;
        // ======================================
        ...
        if (finished == true)
            ...
            return true;
              ...
              // requested
        brakePrevEncoderVals[0] = getEncoderTicks(Motors.LEFT_REAR);
        brakePrevEncoderVals[1] = getEncoderTicks(Motors.RIGHT_REAR);
        if (this.brakePrevEncoderVals.length > 2)
            {
            brakePrevEncoderVals[2] = getEncoderTicks(Motors.LEFT_FRONT);
            brakePrevEncoderVals[3] = getEncoderTicks(Motors.RIGHT_FRONT);
            } // if
        if (this.isDebugOn(debugType.DEBUG_BRAKING) == true)
            {
        ...
        newSpeed[1] = -brakeMotorDirection[1] * this.brakeInitialDirection[1]
                * power;
        transmission.getMotorController(Motors.LEFT_REAR).set(newSpeed[0]);
        transmission.getMotorController(Motors.RIGHT_REAR).set(newSpeed[1]);
        ...
        if (encoders.length >= 4)
            ...
            transmission.getMotorController(Motors.LEFT_FRONT).set(newSpeed[2]);
            transmission.getMotorController(Motors.RIGHT_FRONT)
                    .set(newSpeed[3]);
            if (this.isDebugOn(debugType.DEBUG_BRAKING) == true)
    ...
    public boolean brakeOld(BrakeType type)
    {
        ...
        if (System.currentTimeMillis() - previousBrakeTime > INIT_TIMEOUT)
            ...
            if (transmission.getMotorController(Motors.LEFT_REAR)
                    .getInverted() == true)
                brakeMotorDirection[0] = -1;

            if (transmission.getMotorController(Motors.RIGHT_REAR)
                    .getInverted() == true)
            ...
            if (encoders.length >= 4)
                {
                if (transmission.getMotorController(Motors.LEFT_FRONT)
                        .getInverted() == true)
                    brakeMotorDirection[2] = -1;

                if (transmission.getMotorController(Motors.RIGHT_FRONT)
                        .getInverted() == true)
        ...
        // Set the rear wheels
        transmission.getMotorController(Motors.LEFT_REAR)
                .set(-brakeMotorDirection[0] * power);
        transmission.getMotorController(Motors.RIGHT_REAR)
                .set(-brakeMotorDirection[1] * power);
        // Set the front wheels if it's the right kind of drive
        if (encoders.length >= 4)
            {
            transmission.getMotorController(Motors.LEFT_FRONT)
    ...
    private void brakePrep()
    {
        ...
        if (this.isDebugOn(debugType.DEBUG_BRAKING) == true)
            ...
            System.out.print("present MC power LR RR LF RF = "
                    + transmission.getMotorController(Motors.LEFT_REAR).get()
                    + " "
                    + transmission.getMotorController(Motors.RIGHT_REAR).get());
            if (this.brakeMotorDirection.length > 2)
                System.out.print(" "
                        + transmission.getMotorController(Motors.LEFT_FRONT)
                                .get()
                        + " " + transmission
                                .getMotorController(Motors.RIGHT_FRONT).get());
            System.out.println();
            } // if
              // ==================================
        ...
        // ==================================
        if (this.transmission.getMotorController(Motors.LEFT_REAR)
                .getInverted() == true)
            this.brakeMotorDirection[0] = -1;
        if (this.transmission.getMotorController(Motors.RIGHT_REAR)
                .getInverted() == true)
            this.brakeMotorDirection[1] = -1;
        if (this.isDebugOn(debugType.DEBUG_BRAKING) == true)
        ...
        if (this.brakeMotorDirection.length > 2)
            {
            if (this.transmission.getMotorController(Motors.LEFT_FRONT)
                    .getInverted() == true)
                this.brakeMotorDirection[2] = -1;

            if (this.transmission.getMotorController(Motors.RIGHT_FRONT)
                    .getInverted() == true)
                this.brakeMotorDirection[3] = -1;
            if (this.isDebugOn(debugType.DEBUG_BRAKING) == true)
            ...
            boolean isUsingGyro)
    {
        ...
        if (isUsingGyro == true)
            ...
            {
            int delta = getEncoderTicks(Motors.LEFT)
                    - getEncoderTicks(Motors.RIGHT);
     ...
     */
    public KilroyEncoder getEncoder(Motors encoder)
    {
        switch (encoder)
            {
            case LEFT:
    ...
    public double getEncoderDegreesTurned()
    {
        return Math.toDegrees(
                getEncoderDistanceAverage(Motors.ALL) / turningRadius);
    } // end getEncoderDegreesTurned()
     ...
     */
    public double getEncoderDistanceAverage(Motors encoderGroup)
    {
        double added = 0;
        switch (encoderGroup)
            {
     ...
     */
    public double getEncoderRate(Motors encoderGroup)
    {
        switch (encoderGroup)
            {
            // ===========================================================================
     ...
     */
    public int getEncoderTicks(Motors encoder)
    {
        switch (encoder)
            {
            case LEFT:
     ...
     */
    public TankTransmission getTransmission()
    {
            ...
            double accelerationTimeInSecs, boolean usingGyro)
    {
        ...
        // If we are NOT using the gyro, use the encoders.
            ...
            if (degrees > 0 && Math.abs(getEncoderDistanceAverage(
                    Motors.LEFT)) > degreesToEncoderInches(degrees, true))
                finished = true;
            else
                if (Math.abs(getEncoderDistanceAverage(
                        Motors.RIGHT)) > degreesToEncoderInches(degrees, true))
                    finished = true;
            } // else
     ...
     */
    public void setEncoderDistancePerPulse(double value, Motors encoder)
    {
        switch (encoder)
            {
            case ALL:
            ...
            int directionDegrees)
    {
        ...
        if (this.getEncoderDistanceAverage(Motors.ALL) > inches)
            {
    ...
    public boolean turnDegrees(int angle, double speed)
    {
        ...
        if (this.getEncoderDistanceAverage(
                Motors.ALL) > Math.toRadians(Math.abs(angle)) * turningRadius)
            {
            ...
            boolean usingGyro)
    {
        ...
        if (usingGyro && Math.abs(this.gyro.getAngle()) > Math.abs(degrees)
            ...
            System.out.println("Distance Traveled = "
                    + this.getEncoderDistanceAverage(Motors.ALL));
            ...
            if (!usingGyro && this.getEncoderDistanceAverage(
                    Motors.ALL) > degreesToEncoderInches(
                            Math.abs(degrees) - turnDegreesFudgeFactor, true))
                {
                this.transmission.stop();
                turnDegreesInit = true;
    ...
    private final TankTransmission transmission;
